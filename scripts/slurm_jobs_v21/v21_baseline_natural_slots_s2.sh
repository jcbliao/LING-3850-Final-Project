#!/bin/bash
#SBATCH --job-name=v21_baseline_natural_slots_s2
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/sweep_v21/v21_baseline_natural_slots_s2.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/sweep_v21/v21_baseline_natural_slots_s2.yaml')); print(c.get('save_dir', '../checkpoints'))")
