#!/bin/bash
#SBATCH --job-name=v21_baseline_oversample_slots_s1
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/sweep_v21/v21_baseline_oversample_slots_s1.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/sweep_v21/v21_baseline_oversample_slots_s1.yaml')); print(c.get('save_dir', '../checkpoints'))")
