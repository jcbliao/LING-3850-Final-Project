#!/bin/bash
#SBATCH --job-name=v10a_oversample
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v10a_oversample.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v10a_oversample.yaml')); print(c.get('save_dir', '../checkpoints'))")
