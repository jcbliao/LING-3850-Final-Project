#!/bin/bash
#SBATCH --job-name=v22a_transducer_tf
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v22a_transducer_tf.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v22a_transducer_tf.yaml')); print(c.get('save_dir', '../checkpoints'))")
