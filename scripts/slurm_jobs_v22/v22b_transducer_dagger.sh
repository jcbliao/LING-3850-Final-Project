#!/bin/bash
#SBATCH --job-name=v22b_transducer_dagger
...
# Clear inherited conda env-vars from the submit shell so `conda activate`
# resolves env names against the default path on the compute node, not a
# stale or relative override.
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v22b_transducer_dagger.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v22b_transducer_dagger.yaml')); print(c.get('save_dir', '../checkpoints'))")
