#!/bin/bash
#SBATCH --job-name=v23a_retrieval_hmnt
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v23a_retrieval_hmnt.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v23a_retrieval_hmnt.yaml')); print(c.get('save_dir', '../checkpoints'))")
