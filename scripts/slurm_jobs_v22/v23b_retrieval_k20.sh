#!/bin/bash
#SBATCH --job-name=v23b_retrieval_k20
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v23b_retrieval_k20.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v23b_retrieval_k20.yaml')); print(c.get('save_dir', '../checkpoints'))")
