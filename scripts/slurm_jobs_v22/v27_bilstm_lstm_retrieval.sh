#!/bin/bash
#SBATCH --job-name=v27_bilstm_lstm_retrieval
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/v27_bilstm_lstm_retrieval.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/v27_bilstm_lstm_retrieval.yaml')); print(c.get('save_dir', '../checkpoints'))")
