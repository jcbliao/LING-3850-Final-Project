#!/bin/bash
#SBATCH --job-name=v29_lstm_no_s3
...
cd "${PROJECT_DIR}/src"
python train.py --config "${PROJECT_DIR}/configs/sweep_v29/lstm_no_s3.yaml" 2>&1

SAVE_DIR=$(python -c "import yaml; c=yaml.safe_load(open('${PROJECT_DIR}/configs/sweep_v29/lstm_no_s3.yaml')); print(c.get('save_dir', '../checkpoints'))")
